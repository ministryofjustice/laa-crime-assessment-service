...
public class BadRequestLoggingAdvice {
    ...
    public ResponseEntity<Void> handleValidation(MethodArgumentNotValidException ex,
        ...
        log.info("400 Validation failed on {} {}: {}",
                req.getMethod(), req.getRequestURI(), errors);
    ...
    public ResponseEntity<Void> handleNotReadable(HttpMessageNotReadableException ex,
        ...
        log.info("400 Could not read request body on {} {}: {}",
                req.getMethod(), req.getRequestURI(), rootCauseMessage(ex), ex);
    ...
    public ResponseEntity<Void> handleMissingParam(MissingServletRequestParameterException ex,
            HttpServletRequest req) {
        log.info("400 Missing request parameter on {} {}: {}", req.getMethod(), req.getRequestURI(),
                ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
    }
