...
class IojAppealControllerTest {
    ...
    private static final String IOJAPPEAL_ID_ENDPOINT = IOJAPPEAL_ENDPOINT + "/{id}";
    ...
    void givenEndpointNotImplemented_whenGetByIdEndpointCalled_thenError() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(IOJAPPEAL_ID_ENDPOINT, TEST_ID))
                .andExpect(status().isNotImplemented());
    }

    @Test
    void givenEndpointNotImplemented_whenCreateByIdEndpointCalled_thenError() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(IOJAPPEAL_ID_ENDPOINT, TEST_ID))
                .andExpect(status().isNotImplemented());
    }
    ...
    void givenIncorrectMethod_whenCreateByIdEndpointCalled_thenError() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.delete(IOJAPPEAL_ID_ENDPOINT, TEST_ID))
                .andExpect(status().isMethodNotAllowed());
        mockMvc.perform(MockMvcRequestBuilders.patch(IOJAPPEAL_ID_ENDPOINT, TEST_ID))
                .andExpect(status().isMethodNotAllowed());
        mockMvc.perform(MockMvcRequestBuilders.put(IOJAPPEAL_ID_ENDPOINT, TEST_ID))
                .andExpect(status().isMethodNotAllowed());
    }
